export function createProjectOverlay() {
  ...
  root.innerHTML = `
    <div style="text-shadow:0 0 18px rgba(255,255,255,.8)">
      ...
      <p style="margin:5px 0;opacity:.75">СОЗДАЙ ВПЕЧАТЛЕНИЕ ВМЕСТЕ</p>
    </div>
    ...
    <div id="atom-card" style="position:absolute;right:24px;bottom:30px;padding:18px 22px;border-radius:18px;background:rgba(0,0,0,.35);backdrop-filter:blur(8px);opacity:0;transition:.3s">
      <div style="font-size:12px;opacity:.7">ЭТАП ПУТИ</div>
      <div id="atom-name" style="font-size:28px">АТОМ</div>
      <div id="atom-description">Открой следующий этап</div>
      <div id="atom-action" style="margin-top:10px;opacity:.8"></div>
    </div>
  ...
  window.addEventListener('journey-step-active', (event) => {
    ...
    const action = root.querySelector('#atom-action');
    const step = event.detail.step;

    name.textContent = step.title || event.detail.label || 'ЭТАП';
    description.textContent = step.prompt || 'Следующий шаг открыт';
    action.textContent = step.action ? `→ ${step.action}` : '';
  ...
  window.addEventListener('atom-active', (event) => {
    const card = root.querySelector('#atom-card');
    const name = root.querySelector('#atom-name');
    if (!root.querySelector('#atom-name').textContent || root.querySelector('#atom-name').textContent === 'АТОМ') {
      name.textContent = event.detail.title || event.detail.type || 'АТОМ';
    }
    card.style.opacity = '1';
  });
